				...
				<cfloop from="1" to="#arrayLen(rc.arStandings)#" index="local.itm">
					...
					<tr<cfif rc.nCurrentUser eq local.nUserID> class="highlight-user"</cfif>>
						...
						<cfloop from="#arrayLen(local.arWeeks)#" to="1" step="-1" index="local.y">
							...
							<td data-order="#((!isNull(local.stUserWeekData[local.nWeekID].nPlace)) ? local.stUserWeekData[local.nWeekID].nPlace : 9000)#">#((!isNull(local.stUserWeekData[local.nWeekID].nPoints)) ? local.stUserWeekData[local.nWeekID].nPoints : 0)#
