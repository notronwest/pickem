				...
				if( oHomeTeam.getNTeamID() gt 0 ){
					...
					if( oAwayTeam.getNTeamID() > 0 ){
						...
						if( !isNull(rc.arGames[itm].sGameDateTime) && isDate(rc.arGames[itm].sGameDateTime) ) {
							rc.arGames[itm].sGameDateTime = dateTimeFormat(rc.arGames[itm].sGameDateTime, 'yyyy-mm-dd H:nn');
						}
						if( !isNull(rc.arGames[itm].dtLock) && isDate(rc.arGames[itm].dtLock) ) {
							rc.arGames[itm].dtLock = dateTimeFormat(rc.arGames[itm].dtLock, 'yyyy-mm-dd H:nn');
						}
						// save the game
