...
public void function save(rc){
	...
	param name="rc.nBonus" default="0";
	rc.bIsDialog = false;
	oWeek = variables.weekGateway.update((rc.oWeek.getNWeekID() eq 0) ? entityNew("week") : rc.oWeek, { sName = rc.sName, dStartDate = dateFormat(rc.dStartDate, 'yyyy-mm-dd'), dEndDate = dateFormat(rc.dEndDate, 'yyyy-mm-dd'), lstSports = rc.lstSports, nSeasonID = rc.nCurrentSeasonID, nBonus = rc.nBonus, dPicksDue = dateFormat(rc.dPicksDue, 'yyyy-mm-dd'), tPicksDue = timeFormat(rc.tPicksDue, "HH:mm"), nWeekNumber = rc.nWeekNumber });
